...
RxVertexArray::RxVertexArray()
{
}

std::size_t RxVertexArray::stride() const
{
	return getVertexSize();
}

const void* RxVertexArray::data() const
{
	return reinterpret_cast<const void*>(&this->_data[0]);
}

void RxVertexArray::addAttribute(int32_t componentByteSize, int32_t numComponents, RxVertexDecl::AttributeHint hint)
{
	format.attributes.push_back(RxVertexDecl::Attribute(componentByteSize, numComponents, hint));
}

void RxVertexArray::setVertexAttribute(int32_t vertexIndex, int32_t attributeIndex, void* data)
{
...
int32_t RxVertexArray::getAttributeOffset(int32_t attributeIndex) const
{
...
int32_t RxVertexArray::getAttributeSize(int32_t attributeIndex) const
{
	return format.attributes[attributeIndex].size * format.attributes[attributeIndex].numComponents;
}

int32_t RxVertexArray::getVertexSize() const
{
...
void RxVertexArray::allocateData(int32_t vertexCount)
{
...
bool RxVertexArray::isVertexEqual(int32_t index1, int32_t index2)
{
...
char* RxVertexArray::getAttribute(int32_t attributeIndex, int32_t vertexIndex)
{
	return &_data[0] + (getVertexSize() * vertexIndex) + getAttributeOffset(attributeIndex);
}

void RxVertexArray::removeLast()
{
	_data.resize(_data.size() - getVertexSize());
	count--;
}

int32_t RxVertexArray::getMemorySize() const
{
	return _data.size();
}

void RxVertexArray::swapVertices(int32_t index, int32_t goesTo)
{
...
void RxVertexArray::removeDuplicateVertices(RxVertexArray& varray, RxIndexArray& iarray)
{
