...
#include <Nephilim/Game/GameEngine.h>
...
void RendererGLES::drawDebugTriangleFan(Vector2* vlist, int vcount, Color color){
	#ifdef NEPHILIM_GLES_V1
	glPushMatrix();
	glEnable(GL_BLEND);
	glBlendFunc (GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
...
void RendererGLES::drawDebugLine(Vector2 begin, Vector2 end, Color color){
	#ifdef NEPHILIM_GLES_V1
	glLoadIdentity();
	glPushMatrix();
...
/*
void RendererGLES::drawDebugCircle(Vector2 center, float radius, Vector2 axis, Color color){
	...
	Vector2* vlist = new Vector2[16];
	for (int i = 0; i  < k_segments; ++i)
	{		
		vlist[i] = center + Vector2(cosf(theta), sinf(theta)) * radius;
		theta += k_increment;
	}
	glEnableClientState(GL_VERTEX_ARRAY);
	glVertexPointer(2, GL_FLOAT, 0, &vlist[0]);
