...
token get_tok(const std::string& program_source, int& i)
{
...
public:
    ...
    {
        while (next().type == RZTK_LINECOMMENT);
        return *curr;
    }
...
module parser::build(const std::string& src)
{
    lexer lx;
    lx.analyze(src);
    ...
    {
        std::cout << "i: " << i << ": " << lx.tokens[i].text << "      " << lx.tokens[i].type << std::endl;
    }

    module m;
    parse_state state {lx.tokens};

    while (state.currentType() != RZTK_EOF)
    {
        if (state.currentType() == RZTK_FUNCTION)
        {
            m << parseFunction(state);
        }
        else
        {
            state.nextSkipComment();
        }
    }
...
function parser::parseFunction(parse_state& ps)
{
    assert(ps.currentType() == RZTK_FUNCTION);
    ps.next();

    std::string name = ps.text();

    ps.next(3);

    std::cout << "parseFunction: " << name << std::endl;
